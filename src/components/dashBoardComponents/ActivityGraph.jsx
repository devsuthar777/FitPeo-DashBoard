    ...
    <div className='w-[65%] bg-richblack-800 h-60 rounded-md px-4 py-2 '>
        ...
        <div className='relative h-full flex flex-col'>
            <ul className='flex flex-col gap-3 mt-8'>
                {
                    acitivityArray.map((data,index) => <li 
                    className='flex items-center gap-4'
                    key={index}>
                        <p className='w-9 text-richblack-200'>{data}</p> <div className='w-full h-[2px] bg-richblack-500'></div>
                    </li>).reverse()
                }
            </ul>
            <ul className='flex text-richblack-200 justify-between ml-14'>
                {
                    horioztalArray.map((data,index)=><li>
                        {data}
                    </li>)
                }
            </ul>
